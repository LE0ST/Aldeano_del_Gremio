...
CANAL_EVENTOS_ID = 920686951546355764
ROL_STAFF_ID = 778414374296617041
...
@bot.event
async def on_ready():
    print(f"✅ ¡El bot {bot.user} ha despertado en el Gremio!")

    # Inicia el loop solo si aún no está ejecutándose
    if not evento_aleatorio.is_running():
        evento_aleatorio.start()

# =========================
# LOOP DE EVENTOS ALEATORIOS
# =========================

# Cambia seconds=15 por hours=4 cuando termines las pruebas
@tasks.loop(seconds=15)
async def evento_aleatorio():

    # Obtiene el canal donde aparecerán los eventos
    canal = bot.get_channel(CANAL_EVENTOS_ID)

    # Si el canal no existe o el bot no lo encuentra
    if not canal:
        print("❌ No se encontró el canal de eventos.")
        return

    # Selecciona un evento aleatorio de la lista
    evento = random.choice(EVENTOS)

    # Crea el embed del evento
    embed = discord.Embed(
        title=evento["titulo"],
        description=(
            f"{evento['desc']}\n\n"
            "⏳ *Tienes 45 segundos para actuar...*"
        ),
        color=evento["color"]
    )
    ...
    # Envía el evento al canal
    mensaje = await canal.send(embed=embed)

    # Agrega la reacción automáticamente
    await mensaje.add_reaction(evento["emoji"])

    # Espera para que los usuarios reaccionen
    await asyncio.sleep(10)

    # Obtiene nuevamente el mensaje actualizado
    mensaje = await canal.fetch_message(mensaje.id)

    # Lista de participantes válidos
    participantes = []

    # Revisa todas las reacciones del mensaje
    for reaction in mensaje.reactions:

        # Verifica que sea la reacción correcta
        if str(reaction.emoji) == evento["emoji"]:

            # Recorre usuarios que reaccionaron
            async for user in reaction.users():
    ...
    # Si hubo participantes
    if participantes:

        # Elige un ganador aleatorio
        ganador = random.choice(participantes)

        # Genera recompensa aleatoria
        premio_kakera = random.randint(
            evento["min_k"],
            evento["max_k"]
        )
        ...
        embed_resultado = discord.Embed(
            ...
            description=(
                "El destino ha hablado...\n\n"
                f"🎉 **{ganador.mention}** tuvo éxito en la misión.\n"
                f"💎 **Botín obtenido:** "
                f"`{premio_kakera} Kakera` "
                "<:ka_amarillo:1506025670734516406>\n\n"
                "*Alto Mando, por favor entreguen la recompensa "
                "en la instancia correspondiente.*"
            ),
            color=0xF1C40F
        )

        # Foto de perfil del ganador
        embed_resultado.set_thumbnail(
            url=ganador.display_avatar.url
        )
